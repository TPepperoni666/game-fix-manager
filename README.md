...
| **📁 Scan SD for Games** | Walks the SD card's `Games/` folder and matches each subfolder to a recipe (by name, aliases, install-dir names, or marker files). With your confirmation, writes every match into `~/.config/gfm/config.json` so future Applies never ask you for those paths. Unmatched folders are surfaced so you know what needs a recipe alias. |
| **🔌 Connect NAS Payloads** | One-shot setup of an SMB automount for your off-git payload share (Eclipse mods etc.). Asks for host/share/credentials, installs a systemd automount that survives reboots and won't hang boot when the NAS is offline, and points GFM at it automatically. Deck/Linux only; re-run after a reimage. Also available as `tools/deck-smb-automount.sh`. |
...
| `python3 gfm.py scan` | scan SD Games/ and auto-populate game paths |
| `python3 gfm.py setup-nas` | install the NAS payload automount |
